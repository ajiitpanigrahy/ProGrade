...
import {
  ...
  RefreshCcw, Loader2, CheckCircle2, AlertCircle // 🌟 Added new icons
} from 'lucide-react';
import logo from '../assets/logo.svg';
// 🌟 Make sure to import your axiosClient! Adjust path as necessary.
import { axiosClient } from '../api/axiosClient'; 

export default function LandingPage() {
  const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false);

  // 🌟 FIX 1: THEME TOGGLE WITH LOCAL STORAGE
  ...
  const handleContactSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Calls the public backend endpoint we will create next
      await axiosClient.post('/public/contact', contactForm);
      setContactStatus({ loading: false, error: '', success: 'Message sent successfully! Please check your email.' });
      setContactForm({ firstName: '', lastName: '', email: '', message: '' });
      setTimeout(() => setContactStatus(prev => ({ ...prev, success: '' })), 6000);
    } catch (err: any) {
      setContactStatus({ loading: false, error: err.response?.data?.error || 'Failed to send message.', success: '' });
    }
  };
  ...
  return (
    <div className="scroll-smooth bg-white dark:bg-[#0a0514] font-sans text-gray-900 dark:text-gray-100 transition-colors duration-300">
      ...
      <nav className="fixed w-full bg-[#0f0a1c] border-b border-purple-900/50 z-50 top-0 transition-all">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-20">
            ...
            <div className="hidden lg:flex space-x-8">
              {navLinks.map((link) => (
                <button key={link.name} onClick={() => scrollToSection(link.id)} className="relative cursor-pointer text-gray-300 hover:text-white font-medium transition-colors group py-2 focus:outline-none">
                  {link.name}
            ...
            <div className="hidden lg:flex items-center space-x-4">
              {/* 🌟 FIX 2: REFRESH ICON */}
              <button onClick={() => window.location.reload()} title="Refresh Page" className="cursor-pointer p-2 text-gray-400 hover:text-white transition-colors group focus:outline-none">
                <RefreshCcw className="w-5 h-5 group-hover:animate-[spin_0.5s_linear]" />
              </button>
              
              <button onClick={toggleTheme} title="Toggle Theme" className="cursor-pointer p-2 text-gray-400 hover:text-fuchsia-400 transition-colors focus:outline-none">
                {isDarkMode ? <Sun className="w-5 h-5" /> : <Moon className="w-5 h-5" />}
              </button>
              
              <div className="w-px h-6 bg-purple-900/50 mx-2"></div>
              
              <Link to="/login" className="cursor-pointer text-purple-300 font-semibold hover:text-white transition-colors px-2">
                Login
              </Link>
            ...
            <div className="lg:hidden flex items-center gap-2">
              <button onClick={() => window.location.reload()} className="cursor-pointer p-2 text-gray-400 hover:text-white focus:outline-none">
                <RefreshCcw className="w-5 h-5" />
              </button>
              <button onClick={toggleTheme} className="cursor-pointer p-2 text-gray-400 hover:text-fuchsia-400 focus:outline-none">
                {isDarkMode ? <Sun className="w-5 h-5" /> : <Moon className="w-5 h-5" />}
              </button>
              <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="cursor-pointer text-gray-300 hover:text-white focus:outline-none p-2">
        ...
        {isMobileMenuOpen && (
          <div className="lg:hidden absolute top-20 left-0 w-full bg-[#0f0a1c] border-b border-purple-900/50 shadow-2xl flex flex-col py-3 px-5 space-y-1 sm:space-y-2">
            {navLinks.map((link) => (
              <button key={link.name} onClick={() => scrollToSection(link.id)} className="cursor-pointer text-left text-gray-300 hover:text-white font-medium py-2 text-base sm:text-lg focus:outline-none transition-colors">
                {link.name}
              </button>
            ))}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-12 sm:gap-16 items-center">
              ...
              <div className="space-y-6 sm:space-y-8">
                ...
                <div className="flex items-start gap-4 sm:gap-5">
                  ...
                  <div>
                    ...
                    <p className="text-sm sm:text-base text-gray-400 mt-1">+91 1234 567 890</p>
                  </div>
                </div>
                <div className="flex items-start gap-4 sm:gap-5">
                  <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-purple-800/50 flex items-center justify-center shrink-0 border border-purple-700/50">
            ...
            <div className="bg-white dark:bg-[#1a0d36] rounded-2xl p-6 sm:p-8 md:p-10 text-gray-900 dark:text-white shadow-2xl relative transition-colors duration-300">
              ...
              {contactStatus.error && (
                <div className="mb-4 p-3 rounded-xl bg-red-500/10 border border-red-500/30 text-red-500 text-xs font-bold flex items-center gap-2">
                  <AlertCircle className="w-4 h-4 shrink-0" /> {contactStatus.error}
                </div>
              )}
              {contactStatus.success && (
                <div className="mb-4 p-3 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-500 text-xs font-bold flex items-center gap-2">
                  <CheckCircle2 className="w-4 h-4 shrink-0" /> {contactStatus.success}
                </div>
              )}

              <form onSubmit={handleContactSubmit} className="space-y-4 sm:space-y-5 relative z-10">
                <div className="grid sm:grid-cols-2 gap-4 sm:gap-5">
                  <div>
                    <label className="block text-xs sm:text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5">First Name</label>
                    <input required value={contactForm.firstName} onChange={(e) => setContactForm({...contactForm, firstName: e.target.value})} type="text" className="w-full px-3 sm:px-4 py-2.5 sm:py-3 bg-gray-50 dark:bg-[#0f0a1c] border border-gray-200 dark:border-purple-900/50 rounded-xl focus:bg-white dark:focus:bg-[#150a29] focus:ring-2 focus:ring-purple-600 focus:border-transparent outline-none transition-all text-sm" placeholder="Ajit" />
                  </div>
                  <div>
                    <label className="block text-xs sm:text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5">Last Name</label>
                    <input required value={contactForm.lastName} onChange={(e) => setContactForm({...contactForm, lastName: e.target.value})} type="text" className="w-full px-3 sm:px-4 py-2.5 sm:py-3 bg-gray-50 dark:bg-[#0f0a1c] border border-gray-200 dark:border-purple-900/50 rounded-xl focus:bg-white dark:focus:bg-[#150a29] focus:ring-2 focus:ring-purple-600 focus:border-transparent outline-none transition-all text-sm" placeholder="Panigrahy" />
                  </div>
                </div>
                <div>
                  <label className="block text-xs sm:text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5">Work Email</label>
                  <input required value={contactForm.email} onChange={(e) => setContactForm({...contactForm, email: e.target.value})} type="email" className="w-full px-3 sm:px-4 py-2.5 sm:py-3 bg-gray-50 dark:bg-[#0f0a1c] border border-gray-200 dark:border-purple-900/50 rounded-xl focus:bg-white dark:focus:bg-[#150a29] focus:ring-2 focus:ring-purple-600 focus:border-transparent outline-none transition-all text-sm" placeholder="ajit@company.com" />
                </div>
                <div>
                  <label className="block text-xs sm:text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5">How can we help?</label>
                  <textarea required value={contactForm.message} onChange={(e) => setContactForm({...contactForm, message: e.target.value})} rows={4} className="w-full px-3 sm:px-4 py-2.5 sm:py-3 bg-gray-50 dark:bg-[#0f0a1c] border border-gray-200 dark:border-purple-900/50 rounded-xl focus:bg-white dark:focus:bg-[#150a29] focus:ring-2 focus:ring-purple-600 focus:border-transparent outline-none transition-all resize-none text-sm" placeholder="Tell us about your requirements..."></textarea>
                </div>
                <button type="submit" disabled={contactStatus.loading} className="flex justify-center items-center gap-2 cursor-pointer w-full py-3 sm:py-4 bg-purple-600 text-white rounded-xl font-bold text-base sm:text-lg hover:bg-purple-700 shadow-lg shadow-purple-200 dark:shadow-purple-900/50 transition-all active:scale-[0.98] disabled:opacity-50">
                  {contactStatus.loading ? <Loader2 className="w-5 h-5 animate-spin"/> : 'Submit Request'}
                </button>
              </form>
            </div>
          </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid sm:grid-cols-2 md:grid-cols-4 gap-10 sm:gap-12">
          ...
          <div>
            ...
            <ul className="space-y-3 sm:space-y-4 text-xs sm:text-sm flex flex-col items-start">
              <li><button className="relative cursor-pointer text-gray-400 hover:text-white transition-colors group focus:outline-none">Features<span className="absolute -bottom-1 left-0 w-0 h-[1.5px] bg-purple-500 transition-all duration-300 ease-out group-hover:w-full"></span></button></li>
              <li><button className="relative cursor-pointer text-gray-400 hover:text-white transition-colors group focus:outline-none">Enterprise Pricing<span className="absolute -bottom-1 left-0 w-0 h-[1.5px] bg-purple-500 transition-all duration-300 ease-out group-hover:w-full"></span></button></li>
              <li><button onClick={() => scrollToSection('about')} className="relative cursor-pointer text-gray-400 hover:text-white transition-colors group focus:outline-none">About Us<span className="absolute -bottom-1 left-0 w-0 h-[1.5px] bg-purple-500 transition-all duration-300 ease-out group-hover:w-full"></span></button></li>
            </ul>
          </div>
          {/* Links Col 2 */}
          <div>
            ...
            <ul className="space-y-3 sm:space-y-4 text-xs sm:text-sm flex flex-col items-start">
              <li><Link to="/privacy" className="relative cursor-pointer text-gray-400 hover:text-white transition-colors group focus:outline-none">Privacy Policy<span className="absolute -bottom-1 left-0 w-0 h-[1.5px] bg-purple-500 transition-all duration-300 ease-out group-hover:w-full"></span></Link></li>
              <li><Link to="/terms" className="relative cursor-pointer text-gray-400 hover:text-white transition-colors group focus:outline-none">Terms of Service<span className="absolute -bottom-1 left-0 w-0 h-[1.5px] bg-purple-500 transition-all duration-300 ease-out group-hover:w-full"></span></Link></li>
              <li><Link to="/cookies" className="relative cursor-pointer text-gray-400 hover:text-white transition-colors group focus:outline-none">Cookie Policy<span className="absolute -bottom-1 left-0 w-0 h-[1.5px] bg-purple-500 transition-all duration-300 ease-out group-hover:w-full"></span></Link></li>
            </ul>
          </div>
          {/* Socials Col */}
          <div>
            ...
            <div className="flex gap-3 sm:gap-4">
              {/* https://www.instagram.com/ajiitpanigrahy/ */}
              <a href="#" aria-label="Instagram" className="cursor-pointer w-8 h-8 sm:w-10 sm:h-10 bg-purple-900/40 rounded-full flex items-center justify-center hover:bg-purple-600 hover:text-white transition-all border border-purple-800/50">
              ...
              {/* https://github.com/ajiitpanigrahy */}
              <a href="#" aria-label="GitHub" className="cursor-pointer w-8 h-8 sm:w-10 sm:h-10 bg-purple-900/40 rounded-full flex items-center justify-center hover:bg-purple-600 hover:text-white transition-all border border-purple-800/50">
              ...
              {/* https://www.linkedin.com/in/ajitpanigrahy */}
              <a href="#" aria-label="LinkedIn" className="cursor-pointer w-8 h-8 sm:w-10 sm:h-10 bg-purple-900/40 rounded-full flex items-center justify-center hover:bg-purple-600 hover:text-white transition-all border border-purple-800/50">
