...
public class AssessmentController {
	...
	public ResponseEntity<?> getAllAssessments(Authentication auth) {
		...
		// Filter for Educators: Only show their own exams OR exams they are explicitly
		// given access to
		List<Assessment> allowedAssessments = allAssessments.stream().filter(exam -> {
	...
	public ResponseEntity<?> getAssessmentReports(@PathVariable Long assessmentId, Authentication auth) {
		...
		// Security check: Admins, Test Creators, or explicitly allowed Educators can
		// view reports
		boolean isAdmin = auth.getAuthorities().stream().anyMatch(a -> a.getAuthority().equals("ROLE_ADMIN"));
		...
		if (!isAdmin && !isCreator && !isAllowed) {
			return ResponseEntity.status(403)
					.body(Map.of("error", "Access Denied. You do not have permission to view reports for this exam."));
		}

		List<AssessmentSubmission> submissions = submissionRepository
				.findByAssessmentIdOrderByTotalScoreDesc(assessmentId);
		List<MalpracticeLog> fraudLogs = malpracticeLogRepository.findByAssessmentId(assessmentId);

		double avgScore = submissions.isEmpty() ? 0
				: submissions.stream().mapToDouble(AssessmentSubmission::getTotalScore).average().orElse(0);

		return ResponseEntity.ok(Map.of("examTitle", exam.getTitle(), "totalQuestions", exam.getTotalQuestions(),
				"totalStudents", submissions.size(), "averageScore", Math.round(avgScore * 100.0) / 100.0,
				"submissions", submissions, "fraudLogs", fraudLogs));
	}
